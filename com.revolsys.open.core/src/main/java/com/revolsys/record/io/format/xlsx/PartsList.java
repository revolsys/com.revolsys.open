...
public class PartsList {
  ...
  public static void main(final String[] args) throws Exception {
    ...
    final String inputfilepath = System.getProperty("user.dir") + "/sample-docs/xlsx/pivot.xlsm";

    // Open a document from the file system
    // 1. Load the Package - .docx or Flat OPC .xml
    final org.docx4j.openpackaging.packages.OpcPackage xlsxPkg = org.docx4j.openpackaging.packages.OpcPackage
      .load(new java.io.File(inputfilepath));

    // List the parts by walking the rels tree
    final RelationshipsPart rp = xlsxPkg.getRelationshipsPart();
    ...
    for (final WorksheetPart sheet : worksheets) {
      System.out.println(sheet.getPartName().getName());
      final Worksheet ws = sheet.getJaxbElement();
      final SheetData data = ws.getSheetData();
      for (final Row r : data.getRow()) {
        System.out.println("row " + r.getR());
        for (final Cell c : r.getC()) {
  ...
  public static void printInfo(final Part p, final StringBuilder sb, final String indent) {
    sb.append("\n" + indent + "Part " + p.getPartName() + " [" + p.getClass().getName() + "] ");
    if (p instanceof JaxbXmlPart) {
      final Object o = ((JaxbXmlPart)p).getJaxbElement();
  ...
  public static void traverseRelationships(
    final org.docx4j.openpackaging.packages.OpcPackage wordMLPackage, final RelationshipsPart rp,
    final StringBuilder sb, final String indent) {

    // TODO: order by rel id

    for (final Relationship r : rp.getRelationships().getRelationship()) {

      log.info("For Relationship Id=" + r.getId() + " Source is " + rp.getSourceP().getPartName()

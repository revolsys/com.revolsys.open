...
public class IntervalSize {
   ...
   */
  public static boolean isZeroWidth(final double min, final double max) {
    final double width = max - min;
    if (width == 0.0) {
      return true;
    }

    final double maxAbs = Math.max(Math.abs(min), Math.abs(max));
    final double scaledInterval = width / maxAbs;

...
public class FastMath {
  ...
  private static class Split {
    ...
    Split(final double x) {
      this.full = x;
      this.high = Double.longBitsToDouble(Double.doubleToRawLongBits(x) & -1L << 27);
    ...
    public Split multiply(final Split b) {
      ...
      final Split mulBasic = new Split(this.full * b.full);
      final double mulError = this.low * b.low
        - (mulBasic.full - this.high * b.high - this.low * b.high - this.high * b.low);
    ...
    private Split pow(final long e) {
      ...
      if (Double.isNaN(result.full)) {
        if (Double.isNaN(this.full)) {
          ...
          if (FastMath.abs(this.full) < 1) {
            return new Split(FastMath.copySign(0.0, this.full), 0.0);
          } else if (this.full < 0 && (e & 0x1) == 1) {
    ...
    public Split reciprocal() {
      ...
      final double approximateInv = 1.0 / this.full;
      ...
      return Double.isNaN(error) ? splitInv
        : new Split(splitInv.high, splitInv.low - error / this.full);
  ...
  private static final double SINE_TABLE_A[] = {
    ...
    +0.5850973129272461d, +0.6816387176513672d, +0.7675435543060303d, +0.8414709568023682d,
    +0.902267575263977d, +0.9489846229553223d, +0.9808930158615112d, +0.9974949359893799d,
    +0.9985313415527344d,
  };
  ...
  private static final double SINE_TABLE_B[] = {
    ...
    -1.0902938113007961E-8d, -3.9986783938944604E-8d, +4.23719669792332E-8d, -5.207000323380292E-8d,
    +2.800552834259E-8d, +1.883511811213715E-8d, -3.5997360512765566E-9d, +4.116164446561962E-8d,
    +5.0614674548127384E-8d, -1.0129027912496858E-9d,
  };

  /** Cosine table (high bits). */
  private static final double COSINE_TABLE_A[] = {
    +1.0d, +0.9921976327896118d, +0.9689123630523682d, +0.9305076599121094d, +0.8775825500488281d,
    +0.8109631538391113d, +0.7316888570785522d, +0.6409968137741089d, +0.5403022766113281d,
    +0.4311765432357788d, +0.3153223395347595d, +0.19454771280288696d, +0.07073719799518585d,
    -0.05417713522911072d,
  };

  /** Cosine table (low bits). */
  private static final double COSINE_TABLE_B[] = {
    +0.0d, +3.4439717236742845E-8d, +5.865827662008209E-8d, -3.7999795083850525E-8d,
    +1.184154459111628E-8d, -3.43338934259355E-8d, +1.1795268640216787E-8d, +4.438921624363781E-8d,
    +2.925681159240093E-8d, -2.6437112632041807E-8d, +2.2860509143963117E-8d,
  ...
  private static final double TANGENT_TABLE_A[] = {
    ...
    +0.7214844226837158d, +0.9315965175628662d, +1.1974215507507324d, +1.5574076175689697d,
    +2.092571258544922d, +3.0095696449279785d, +5.041914939880371d, +14.101419448852539d,
    -18.430862426757812d,
  };

  /*
   * There are 52 bits in the mantissa of a double. For additional precision, the code splits double
   * numbers into two parts, by clearing the low order 30 bits if possible, and then performs the
   * arithmetic on each half separately.
   */

  /** Tangent table, used by atan() (low bits). */
  private static final double TANGENT_TABLE_B[] = {
    +0.0d, -7.877917738262007E-9d, -2.5857668567479893E-8d, +5.2240336371356666E-9d,
    +5.206150291559893E-8d, +1.8307188599677033E-8d, -5.7618793749770706E-8d,
    +7.848361555046424E-8d, +1.0708593250394448E-7d, +1.7827257129423813E-8d,
    +2.893485277253286E-8d, +3.1660099222737955E-7d, +4.983191803254889E-7d, -3.356118100840571E-7d,
  };

  /** Bits of 1/(2*pi), need for reducePayneHanek(). */
  private static final long RECIP_2PI[] = new long[] {
    0x28be60dbL << 32 | 0x9391054aL, 0x7f09d5f4L << 32 | 0x7d4d3770L,
    0x36d8a566L << 32 | 0x4f10e410L, 0x7f9458eaL << 32 | 0xf7aef158L,
    0x6dc91b8eL << 32 | 0x909374b8L, 0x01924bbaL << 32 | 0x82746487L,
    0x3f877ac7L << 32 | 0x2c4a69cfL, 0xba208d7dL << 32 | 0x4baed121L,
    0x3a671c09L << 32 | 0xad17df90L, 0x4e64758eL << 32 | 0x60d4ce7dL,
    0x272117e2L << 32 | 0xef7e4a0eL, 0xc7fe25ffL << 32 | 0xf7816603L,
    0xfbcbc462L << 32 | 0xd6829b47L, 0xdb4d9fb3L << 32 | 0xc9f2c26dL,
    0xd3d18fd9L << 32 | 0xa797fa8bL, 0x5d49eeb1L << 32 | 0xfaf97c5eL,
  ...
  public static double asinh(double a) {
    ...
    if (a > 0.167) {
      ...
      final double a2 = a * a;
      if (a > 0.097) {
        absAsinh = a * (1 - a2 * (F_1_3 - a2 * (F_1_5 - a2 * (F_1_7 - a2 * (F_1_9 - a2
          * (F_1_11 - a2 * (F_1_13 - a2 * (F_1_15 - a2 * F_1_17 * F_15_16) * F_13_14) * F_11_12)
          * F_9_10) * F_7_8) * F_5_6) * F_3_4) * F_1_2);
      } else if (a > 0.036) {
        absAsinh = a * (1 - a2 * (F_1_3 - a2 * (F_1_5 - a2
          * (F_1_7 - a2 * (F_1_9 - a2 * (F_1_11 - a2 * F_1_13 * F_11_12) * F_9_10) * F_7_8) * F_5_6)
          * F_3_4) * F_1_2);
      } else if (a > 0.0036) {
        absAsinh = a * (1 - a2
          * (F_1_3 - a2 * (F_1_5 - a2 * (F_1_7 - a2 * F_1_9 * F_7_8) * F_5_6) * F_3_4) * F_1_2);
      } else {
  ...
  private static double atan(double xa, double xb, final boolean leftPlane) {
    ...
    /*
     * yb = -0.09001346640161823; yb = yb * epsA2 + 0.11110718400605211; yb = yb * epsA2 +
     * -0.1428571349122913; yb = yb * epsA2 + 0.19999999999273194; yb = yb * epsA2 +
     * -0.33333333333333093; yb = yb * epsA2 * epsA;
     */
  ...
  public static double cbrt(double x) {
    ...
    final double p2 = Double
      .longBitsToDouble(inbits & 0x8000000000000000L | (long)(exp3 + 1023 & 0x7ff) << 52);
  ...
  private static double exp(final double x, final double extra, final double[] hiPrec) {
    ...
    /*
     * Lookup exp(floor(x)). intPartA will have the upper 22 bits, intPartB will have the lower 52
     * bits.
     */
    if (x < 0.0) {
    ...
    /*
     * Get the fractional part of x, find the greatest multiple of 2^-10 less than x and look up the
     * exp function of it. fracPartA will have the upper 22 bits, fracPartB the lower 52 bits.
     */
    ...
    /*
     * epsilon is the difference in x from the nearest multiple of 2^-10. It has a value in the
     * range 0 <= epsilon < 2^-10. Do the subtraction from x as the last step to avoid possible loss
     * of precision.
     */
    final double epsilon = x - (intVal + intFrac / 1024.0);

    /*
     * Compute z = exp(epsilon) - 1.0 via a minimax polynomial. z has full double precision (52
     * bits). Since z < 2^-10, we will have 62 bits of precision when combined with the constant 1.
     * This will be used in the last addition below to get proper rounding.
     */

    /*
     * Remez generated polynomial. Converges on the interval [0, 2^-10], error is less than 0.5 ULP
     */
    ...
    /*
     * Compute (intPartA+intPartB) * (fracPartA+fracPartB) by binomial expansion. tempA is exact
     * since intPartA and intPartB only have 22 bits each. tempB will have 52 bits of precision.
     */
    final double tempA = intPartA * fracPartA;
    final double tempB = intPartA * fracPartB + intPartB * fracPartA + intPartB * fracPartB;

    /*
     * Compute the result. (1+z)(tempA+tempB). Order of operations is important. For accuracy add by
     * increasing size. tempA is exact and much larger than the others. If there are extra bits
     * specified from the pow() function, use them.
     */
  ...
  private static double log(final double x, final double[] hiPrec) {
    ...
    /*
     * double epsilon = x / Double.longBitsToDouble(bits & 0xfffffc0000000000L); epsilon -= 1.0;
     */
    ...
    if (hiPrec != null) {
      ...
      /* split, so now lnz = a */
      /*
       * tmp = aa * 1073741824.0; lnza = aa + tmp - tmp; lnzb = aa - lnza + ab;
       */
      lnza = aa + ab;
      lnzb = -(lnza - aa - ab);
    } else {
    ...
    /*
     * Relative sizes: lnzb [0, 2.33E-10] lnm[1] [0, 1.17E-7] ln2B*exp [0, 1.12E-4] lnza [0, 9.7E-4]
     * lnm[0] [0, 0.692] ln2A*exp [0, 709]
     */

    /*
     * Compute the following sum: lnzb + lnm[1] + ln2B*exp + lnza + lnm[0] + ln2A*exp;
     */
  ...
  private static void reducePayneHanek(final double x, final double result[]) {
     ...
     * PI/2, so use the following steps: 1.) multiply by 4. 2.) do a fixed point muliply by PI/4.
     * 3.) Convert to floating point. 4.) Multiply by 2
     */
    ...
    /* Convert to double */
    final double tmpA = (prod2A >>> 12) / TWO_POWER_52; // High order 52 bits
    final double tmpB = (((prod2A & 0xfffL) << 40) + (prod2B >>> 24)) / TWO_POWER_52 / TWO_POWER_52; // Low
                                                                                                     // bits
  ...
  public static double sinh(double x) {
    ...
    if (x > 0.25) {
      ...
      temp = denom * HEX_40000000;
      final double za = denom + temp - temp;
  ...
  private static double sinQ(final double xa, final double xb) {
    ...
    /*
     * Compute the following sum: result = sintA + costA*sinEpsA + sintA*cosEpsB + costA*sinEpsB +
     * sintB + costB*sinEpsA + sintB*cosEpsB + costB*sinEpsB; Ranges of elements xxxtA 0 PI/2 xxxtB
     * -1.5e-9 1.5e-9 sinEpsA -0.0625 0.0625 sinEpsB -6e-11 6e-11 cosEpsA 1.0 cosEpsB 0 -0.0625
     */
    ...
    /*
     * t = sintA*cosEpsB; c = a + t; d = -(c - a - t); a = c; b = b + d; t = costA*sinEpsB; c = a +
     * t; d = -(c - a - t); a = c; b = b + d;
     */

    b = b + sintB + costB * sinEpsA + sintB * cosEpsB + costB * sinEpsB;
    /*
     * t = sintB; c = a + t; d = -(c - a - t); a = c; b = b + d; t = costB*sinEpsA; c = a + t; d =
     * -(c - a - t); a = c; b = b + d; t = sintB*cosEpsB; c = a + t; d = -(c - a - t); a = c; b = b
     * + d; t = costB*sinEpsB; c = a + t; d = -(c - a - t); a = c; b = b + d;
     */
  ...
  private static double tanQ(final double xa, final double xb, final boolean cotanFlag) {
    ...
    /*
     * double est = (sina+sinb)/(cosa+cosb); double err = (sina - cosa*est) + (sinb - cosb*est); est
     * += err/(cosa+cosb); err = (sina - cosa*est) + (sinb - cosb*est);
     */

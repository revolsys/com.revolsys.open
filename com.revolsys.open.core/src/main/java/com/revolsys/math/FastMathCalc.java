...
class FastMathCalc {
  ...
  private static void buildSinCosTables(final double[] SINE_TABLE_A, final double[] SINE_TABLE_B,
    final double[] COSINE_TABLE_A, final double[] COSINE_TABLE_B, final int SINE_TABLE_LEN,
    final double[] TANGENT_TABLE_A, final double[] TANGENT_TABLE_B) {
   ...
   */
  private static void checkLen(final int expectedLen, final int actual) throws RuntimeException {
    if (expectedLen != actual) {
      throw new RuntimeException(actual + "!=" + expectedLen);
    }
  }
   ...
   */
  static void printarray(final PrintStream out, final String name, final int expectedLen,
    final double[] array) {
    out.println(name + "=");
  ...
  static void printarray(final PrintStream out, final String name, final int expectedLen,
    ...
    out.println(TABLE_START_DECL + " ");
    int i = 0;
    for (final double[] array : array2d) { // "double array[]" causes PMD parsing error
      out.print("        {");
      for (final double d : array) { // assume inner array has very few entries
  ...
  static void splitReciprocal(final double in[], final double result[]) {
    ...
    for (int i = 0; i < 2; i++) {
      ...
      double err = 1.0 - result[0] * in[0] - result[0] * in[1] - result[1] * in[0]
        - result[1] * in[1];
      /* err = 1.0 - err; */

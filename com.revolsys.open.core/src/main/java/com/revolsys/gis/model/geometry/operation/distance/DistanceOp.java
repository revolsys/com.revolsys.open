...
public class DistanceOp {
  ...
  private void computeContainmentDistance(int polyGeomIndex,
    GeometryLocation[] locPtPoly) {
    int locationsIndex = 1 - polyGeomIndex;
    List<Polygon> polys = GeometryUtil.getPolygons(geom[polyGeomIndex]);
    if (polys.size() > 0) {
  ...
  private void computeContainmentDistance(List locs, List polys,
    GeometryLocation[] locPtPoly) {
  ...
  private void computeContainmentDistance(GeometryLocation ptLoc, Polygon poly,
    GeometryLocation[] locPtPoly) {
    Coordinates pt = ptLoc.getCoordinate();
  ...
  private void computeFacetDistance() {
     ...
     */
    List<LineString> lines0 = GeometryUtil.getLines(geom[0]);
    List<LineString> lines1 = GeometryUtil.getLines(geom[1]);

    List<Point> pts0 = GeometryUtil.getPoints(geom[0]);
    List<Point> pts1 = GeometryUtil.getPoints(geom[1]);
  ...
  private void computeMinDistanceLines(List lines0, List lines1,
  ...
  private void computeMinDistancePoints(List<Point> points0,
    List<Point> points1, GeometryLocation[] locGeom) {
  ...
  private void computeMinDistance(LineString line0, LineString line1,
    ...
    if (line0.getBoundingBox().distance(line1.getBoundingBox()) > minDistance)
      return;
    CoordinatesList coord0 = line0;
    CoordinatesList coord1 = line1;
    // brute force approach!
    for (int i = 0; i < coord0.size() - 1; i++) {
      for (int j = 0; j < coord1.size() - 1; j++) {
        double dist = LineSegmentUtil.distance(coord0.get(i),
          coord0.get(i + 1), coord1.get(j), coord1.get(j + 1));
        if (dist < minDistance) {
          minDistance = dist;
          LineSegment seg0 = new LineSegment(coord0.get(i), coord0.get(i + 1));
          LineSegment seg1 = new LineSegment(coord1.get(j), coord1.get(j + 1));
          Coordinates[] closestPt = seg0.closestPoints(seg1);
  ...
  private void computeMinDistance(LineString line, Point pt,
    ...
    if (line.getBoundingBox().distance(pt.getBoundingBox()) > minDistance)
      return;
    CoordinatesList coord0 = line;
    Coordinates coord = pt;
    // brute force approach!
    for (int i = 0; i < coord0.size() - 1; i++) {
      double dist = LineSegmentUtil.distance(coord0.get(i), coord0.get(i + 1),
        coord);
      if (dist < minDistance) {
        minDistance = dist;
        LineSegment seg = new LineSegment(coord0.get(i), coord0.get(i + 1));
        Coordinates segClosestPoint = seg.closestPoint(coord);

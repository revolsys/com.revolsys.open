...
public class GeometryFilter {
  public static boolean acceptEnvelopeIntersects(final BoundingBox envelope, final Geometry geometry) {
    final BoundingBox geometryEnvelope = geometry.getBoundingBox();
    return envelope.intersects(geometryEnvelope);
  }

  public static <T extends Geometry> Filter<T> intersects(final BoundingBox envelope) {

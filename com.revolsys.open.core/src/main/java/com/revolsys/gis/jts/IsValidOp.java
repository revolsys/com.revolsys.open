...
import com.revolsys.jts.operation.valid.ConnectedInteriorTester;
import com.revolsys.jts.operation.valid.IndexedNestedRingTester;
...
public class IsValidOp {
  ...
  private boolean addError(final int errorType, final Coordinates point) {
  ...
  private boolean checkClosedRings(final Polygon poly) {
    boolean valid = true;
    valid &= checkClosedRing(poly.getExteriorRing());
   ...
   */
  private boolean checkHolesInShell(final Polygon polygon,
  ...
  private boolean checkShellsNotNested(final MultiPolygon multiPolygon,
    ...
    for (int i = 0; i < multiPolygon.getNumGeometries(); i++) {
      final Polygon p = (Polygon)multiPolygon.getGeometry(i);
      final LinearRing shell = p.getExteriorRing();
      for (int j = 0; j < multiPolygon.getNumGeometries(); j++) {
        if (i == j) {
          continue;
        }
        final Polygon p2 = (Polygon)multiPolygon.getGeometry(j);
  ...
  private boolean checkValid(final LinearRing g) {
    boolean valid = checkInvalidCoordinates(g.getCoordinateArray());
  ...
  private boolean checkValid(final MultiPoint g) {
    return checkInvalidCoordinates(g.getCoordinateArray());
  }

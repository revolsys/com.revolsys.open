...
import com.revolsys.elevation.tin.tin.TinReader;
...
public class TinProcess extends BaseInOutProcess<Record, Record> {
  ...
  protected void loadTin() {
    ...
    if (this.tin == null) {
      ...
      if (this.tinIn != null) {
        readTinFeatures(tin, lines, this.tinIn);
      }
      if (this.tinReader != null) {
        readTinFeatures(tin, lines, this.tinReader);
      }
      ...
      if (this.tinCache != null) {
        ...
        try {
          ...
          final FileSystemResource resource = new FileSystemResource(tinFile);
          this.tin.writeTriangulatedIrregularNetwork(resource);
        } catch (final Exception e) {
  ...
  private void readTinFeatures(final TriangulatedIrregularNetworkBuilder tin,

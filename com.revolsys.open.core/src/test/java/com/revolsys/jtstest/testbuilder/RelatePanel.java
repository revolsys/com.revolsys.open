...
public class RelatePanel extends JPanel {
  ...
  private final ImageIcon tickIcon = new ImageIcon(this.getClass().getResource(
    "tickShaded.gif"));

  private final ImageIcon crossIcon = new ImageIcon(this.getClass()
    .getResource("crossShaded.gif"));

  private final ImageIcon clearIcon = new ImageIcon(this.getClass()
    .getResource("clear.gif"));
  ...
  private final JLabel jLabel110 = new JLabel();

  private final JLabel jLabel111 = new JLabel();
  ...
  void jbInit() throws Exception {
    ...
    border3 = BorderFactory.createCompoundBorder(
      new TitledBorder(BorderFactory.createLineBorder(Color.gray, 1),
        "Intersection Matrix"), BorderFactory.createEmptyBorder(0, 5, 0, 5));
    ...
    jPanel3.add(jPanel1, new GridBagConstraints(100, 100, 1, 1, 0.0, 0.0,
      GridBagConstraints.CENTER, GridBagConstraints.NONE,
      new Insets(0, 0, 0, 0), 0, 0));
    ...
    matrixPanel.add(jLabel11, new GridBagConstraints(2, 1, 2, 1, 0.0, 0.0,
      GridBagConstraints.EAST, GridBagConstraints.NONE, new Insets(0, 5, 0, 5),
      0, 0));
    ...
    matrixPanel.add(jLabel7, new GridBagConstraints(3, 0, 1, 1, 0.0, 0.0,
      GridBagConstraints.SOUTHWEST, GridBagConstraints.NONE, new Insets(5, 0,
        0, 0), 0, 0));
    jPanel1.add(txtBA, new GridBagConstraints(2, 1, 1, 1, 0.0, 0.0,
      GridBagConstraints.NORTHWEST, GridBagConstraints.BOTH, new Insets(2, 2,
        2, 2), 0, 0));
    jPanel1.add(jLabel22, new GridBagConstraints(1, 1, 1, 1, 0.0, 0.0,
      GridBagConstraints.WEST, GridBagConstraints.NONE, new Insets(0, 0, 0, 5),
      0, 0));
    ...
    jPanel1.add(jLabel20, new GridBagConstraints(1, 0, 1, 2, 0.0, 0.0,
      GridBagConstraints.NORTHWEST, GridBagConstraints.NONE, new Insets(0, 2,
        0, 3), 0, 0));
    ...
    this.add(jPanel2, new GridBagConstraints(0, 1, 1, 1, 0.0, 0.0,
      GridBagConstraints.SOUTHWEST, GridBagConstraints.BOTH, new Insets(0, -6,
        0, 6), 0, 0));
    ...
    predicates.add(jLabel19, new GridBagConstraints(0, 8, 1, 1, 0.0, 0.0,
      GridBagConstraints.SOUTHEAST, GridBagConstraints.NONE, new Insets(0, 0,
        0, 10), 0, 0));
  ...
  public void runTests() {
    if (testCase.getGeometry(0) != null && testCase.getGeometry(1) != null) {
      final IntersectionMatrix im = testCase.getIM();
      final IntersectionMatrix imBA = new IntersectionMatrix(im);
      // IntersectionMatrix expectedIm = new
      // IntersectionMatrix(expectedImTextField.getText());
      imBA.transpose();
      setRelateLabel(relateII,
        im.get(IntersectionMatrix.INTERIOR, IntersectionMatrix.INTERIOR));
      setRelateLabel(relateIB,
        im.get(IntersectionMatrix.INTERIOR, IntersectionMatrix.BOUNDARY));
      setRelateLabel(relateIE,
        im.get(IntersectionMatrix.INTERIOR, IntersectionMatrix.EXTERIOR));
      setRelateLabel(relateBI,
        im.get(IntersectionMatrix.BOUNDARY, IntersectionMatrix.INTERIOR));
      setRelateLabel(relateBB,
        im.get(IntersectionMatrix.BOUNDARY, IntersectionMatrix.BOUNDARY));
      setRelateLabel(relateBE,
        im.get(IntersectionMatrix.BOUNDARY, IntersectionMatrix.EXTERIOR));
      setRelateLabel(relateEI,
        im.get(IntersectionMatrix.EXTERIOR, IntersectionMatrix.INTERIOR));
      setRelateLabel(relateEB,
        im.get(IntersectionMatrix.EXTERIOR, IntersectionMatrix.BOUNDARY));
      setRelateLabel(relateEE,
        im.get(IntersectionMatrix.EXTERIOR, IntersectionMatrix.EXTERIOR));
      // tickCrossLabel.setIcon(im.matches(expectedIm.toString()) ? tickIcon :
      // crossIcon);
      txtAB.setText(im.toString());
      txtBA.setText(imBA.toString());

      setPredicate(equalsAB, im.isEquals(
        testCase.getGeometry(0).getDimension(), testCase.getGeometry(1)
          .getDimension()));
      setPredicate(disjointAB, im.isDisjoint());
      setPredicate(intersectsAB, im.isIntersects());
      setPredicate(touchesAB, im.isTouches(testCase.getGeometry(0)
        .getDimension(), testCase.getGeometry(1).getDimension()));
      setPredicate(crossesAB, im.isCrosses(testCase.getGeometry(0)
        .getDimension(), testCase.getGeometry(1).getDimension()));
      setPredicate(withinAB, im.isWithin());
      setPredicate(containsAB, im.isContains());
      setPredicate(overlapsAB, im.isOverlaps(testCase.getGeometry(0)
        .getDimension(), testCase.getGeometry(1).getDimension()));
      setPredicate(coversAB, im.isCovers());
      setPredicate(coveredByAB, im.isCoveredBy());

      setPredicate(equalsBA, imBA.isEquals(testCase.getGeometry(1)
        .getDimension(), testCase.getGeometry(0).getDimension()));
      setPredicate(disjointBA, imBA.isDisjoint());
      setPredicate(intersectsBA, imBA.isIntersects());
      setPredicate(touchesBA, imBA.isTouches(testCase.getGeometry(1)
        .getDimension(), testCase.getGeometry(0).getDimension()));
      setPredicate(crossesBA, imBA.isCrosses(testCase.getGeometry(1)
        .getDimension(), testCase.getGeometry(0).getDimension()));
      setPredicate(withinBA, imBA.isWithin());
      setPredicate(containsBA, imBA.isContains());
      setPredicate(overlapsBA, imBA.isOverlaps(testCase.getGeometry(1)
        .getDimension(), testCase.getGeometry(0).getDimension()));
  ...
  void setPredicate(final JLabel lbl, final boolean b) {
    final String val = b ? "T" : "F";
    lbl.setText(val);
  }

  void setRelateLabel(final JLabel lbl, final int imValue) {
    buf.setCharAt(0, com.revolsys.jts.geom.Dimension.toDimensionSymbol(imValue));

...
import com.revolsys.jtstest.testrunner.CommandLine;
import com.revolsys.jtstest.testrunner.Option;
import com.revolsys.jtstest.testrunner.OptionSpec;
import com.revolsys.jtstest.testrunner.ParseException;
...
public class JTSTestBuilder {
  ...
  private static CommandLine createCmdLine() {
    ...
    commandLine.addOptionSpec(new OptionSpec(OPT_GEOMFUNC,
      OptionSpec.NARGS_ONE_OR_MORE));
    return commandLine;
  }
  ...
  private static void readArgs(final String[] args) throws ParseException,
    ClassNotFoundException {
    commandLine.parse(args);

    if (commandLine.hasOption(OPT_GEOMFUNC)) {
      final Option opt = commandLine.getOption(OPT_GEOMFUNC);
      for (int i = 0; i < opt.getNumArgs(); i++) {
        final String geomFuncClassname = opt.getArg(i);
        try {
   ...
   */
  private static void setLookAndFeel() throws InterruptedException,
  ...
  private void initFrame() {
    final JTSTestBuilderFrame frame = new JTSTestBuilderFrame();

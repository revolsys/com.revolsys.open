...
public class SnapRoundingTest extends TestCase {
  ...
  private boolean isSnapped(final Coordinate v, final Coordinate p0,
    final Coordinate p1) {
    if (v.equals2D(p0)) {
      return true;
    }
  ...
  private boolean isSnapped(final Coordinate v, final List lines) {
  ...
  boolean isSnapped(final List lines, final double tol) {
    for (int i = 0; i < lines.size(); i++) {
      ...
      for (int j = 0; j < line.getNumPoints(); j++) {
        final Coordinate v = line.getCoordinateN(j);

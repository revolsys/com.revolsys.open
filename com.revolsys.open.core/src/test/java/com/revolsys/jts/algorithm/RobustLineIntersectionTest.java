...
public class RobustLineIntersectionTest extends TestCase {
  public static boolean equals(final Coordinate p0, final Coordinate p1,
  ...
  void checkInputNotAltered(final String wkt1, final String wkt2,
  ...
  void checkIntersection(final Coordinate[] pt,
    final int expectedIntersectionNum, final Coordinate[] expectedIntPt,
    final double distanceTolerance) {
    final LineIntersector li = new RobustLineIntersector();
    li.computeIntersection(pt[0], pt[1], pt[2], pt[3]);

    final int intNum = li.getIntersectionNum();
    assertEquals("Number of intersections not as expected",
      expectedIntersectionNum, intNum);

    if (expectedIntPt != null) {
      assertEquals("Wrong number of expected int pts provided", intNum,
        expectedIntPt.length);
      // test that both points are represented here
      final boolean isIntPointsCorrect = true;
      if (intNum == 1) {
        checkIntPoints(expectedIntPt[0], li.getIntersection(0),
          distanceTolerance);
      } else if (intNum == 2) {
        ...
        if (!(equals(expectedIntPt[0], li.getIntersection(0), distanceTolerance) || equals(
          ...
          checkIntPoints(expectedIntPt[0], li.getIntersection(0),
            distanceTolerance);
          checkIntPoints(expectedIntPt[0], li.getIntersection(1),
            distanceTolerance);
  ...
  void checkIntersection(final String wkt1, final String wkt2,
    final int expectedIntersectionNum, final Coordinate[] intPt,
  ...
  void checkIntersection(final String wkt1, final String wkt2,
    final int expectedIntersectionNum, final String expectedWKT,
    final double distanceTolerance) throws ParseException {
    final LineString l1 = (LineString)this.reader.read(wkt1);
    final LineString l2 = (LineString)this.reader.read(wkt2);
    final Coordinate[] pt = new Coordinate[] {
  ...
  void checkIntPoints(final Coordinate expectedPt, final Coordinate actualPt,
    final double distanceTolerance) {
  ...
  public void testCentralEndpointHeuristicFailure2() throws ParseException {
    checkIntersection(
      ...
      1, "POINT ( -215.22279674875 -158.65425425385 )", 0);
  }
  ...
  public void testGEOS_1() throws ParseException {
    checkIntersection(
      ...
      1, "POINT (588748.2060416829 4518933.945284994)", 0);
  }
  ...
  public void testGEOS_2() throws ParseException {
    checkIntersection(
      ...
      1, "POINT (588733.8306132929 4518925.319423238)", 0);
  }
  ...
  public void testLeduc_1() throws ParseException {
    checkIntersection(
      ...
      1, "POINT (305690.0434123494 254176.46578338774)", 0);
  }

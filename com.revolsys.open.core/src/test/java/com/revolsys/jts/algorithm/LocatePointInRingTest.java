...
public class LocatePointInRingTest extends AbstractPointInRingTest {
  ...
  public LocatePointInRingTest(final String name) {
    super(name);
  }

  @Override
  protected void runPtInRing(final int expectedLoc, final Coordinate pt,
    final String wkt) throws Exception {
    final Geometry geom = this.reader.read(wkt);
    assertEquals(expectedLoc,
      CGAlgorithms.locatePointInRing(pt, geom.getCoordinateArray()));
  }

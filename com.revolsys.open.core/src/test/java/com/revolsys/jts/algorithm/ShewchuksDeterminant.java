 ...
 *   com.revolsys.jts.algorithm.CGAlgorithms.orientationIndex so that the current implementation of that method
...
public class ShewchuksDeterminant {
  ...
  private static double orient2d(final Coordinate pa, final Coordinate pb,
    final Coordinate pc) {
    double detsum;

    final double detleft = (pa.x - pc.x) * (pb.y - pc.y);
    final double detright = (pa.y - pc.y) * (pb.x - pc.x);
  ...
  private static double orient2dadapt(final Coordinate pa, final Coordinate pb,
    final Coordinate pc, final double detsum) {

    final double acx = pa.x - pc.x;
    final double bcx = pb.x - pc.x;
    final double acy = pa.y - pc.y;
    final double bcy = pb.y - pc.y;
    ...
    final double acxtail = Two_Diff_Tail(pa.x, pc.x, acx);
    final double bcxtail = Two_Diff_Tail(pb.x, pc.x, bcx);
    final double acytail = Two_Diff_Tail(pa.y, pc.y, acy);
    final double bcytail = Two_Diff_Tail(pb.y, pc.y, bcy);
   ...
   */
  public static int orientationIndex(final Coordinate p1, final Coordinate p2,
    final Coordinate q) {
   ...
   */
  public static int orientationIndexFilter(final Coordinate pa,
    final Coordinate pb, final Coordinate pc) {
    double detsum;

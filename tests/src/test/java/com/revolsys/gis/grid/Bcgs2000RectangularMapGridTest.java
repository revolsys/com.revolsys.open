...
public class Bcgs2000RectangularMapGridTest extends
  ...
  protected void doTestBcgs20000ByName(
    ...
    for (int number = 1; number <= 100; number++) {
      final double lon = parentLon - GridUtil.getNumberCol100(number)
        * TILE_WIDTH;
      final double lat = parentLat + GridUtil.getNumberRow100(number)
        * TILE_HEIGHT;

      final String tileName = parentTileName + "."
        + GridUtil.formatSheetNumber100(number);

      doTestBcgs2000ByName(tileName, lon, lat);
    }
  }

  protected void doTestBcgs2000ByName(
    final String parentTileName,
    final double parentLon,
    final double parentLat) {

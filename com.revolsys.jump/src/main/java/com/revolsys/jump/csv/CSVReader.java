  ...
  private String[] parseLine(String nextLine) throws IOException {
    ...
    do {
      ...
      for (int i = 0; i < nextLine.length(); i++) {
        ...
        if (c == quotechar) {
          // this gets complex... the quote may end a quoted block, or escape
          // another quote.
          ...
          if (inQuotes // we are in quotes, therefore there can be escaped
            ...
            // to check.
            && nextLine.charAt(i + 1) == quotechar) { // ..and that char. is a
            // quote also.
            // we have two quote chars in a row == one quote char, so consume
            // them both and
            ...
            if (i > 2 // not on the begining of the line
              && nextLine.charAt(i - 1) != this.separator // not at the begining
              // of an escape
              // sequence
              && nextLine.length() > (i + 1)
    ...
    if (sb.length() > 0 || tokensOnThisLine.size() > 0) {
      tokensOnThisLine.add(sb.toString());
    }

...
public class ArcSdeBinaryGeometryQueryIterator extends
	...
	private void closeSeQuery() {
		try {
			...
			LoggerFactory.getLogger(ArcSdeBinaryGeometryQueryIterator.class)
					.error("Unable to close query", e);
		}
	}
	...
	protected void doInit() {
		String tableName = dataStore.getDatabaseQualifiedTableName(query
				.getTypeName());
		...
		if (metaData != null) {
			tableName = dataStore.getDatabaseQualifiedTableName(metaData
					.getPath());
		}
		try {
			...
			if (attributeNames.isEmpty()) {
				...
				attributeNames.addAll(metaData.getAttributeNames());
			} else {
			...
			String[] columnNames = attributeNames.toArray(new String[0]);
			seQuery = new SeQuery(sdeConnection, columnNames, sqlConstruct);
			BoundingBox boundingBox = query.getBoundingBox();
			if (boundingBox != null) {
				SeLayer layer = new SeLayer(sdeConnection, tableName, metaData.getGeometryAttributeName());
				
				final GeometryFactory geometryFactory = metaData
						.getGeometryFactory();
				boundingBox = boundingBox.convert(geometryFactory);
				final SeEnvelope envelope = new SeEnvelope(
				...
				SeShapeFilter filter = new SeShapeFilter(tableName,
						metaData.getGeometryAttributeName(), shape,
						SeFilter.METHOD_ENVP);
				seQuery.setSpatialConstraints(SeQuery.SE_SPATIAL_FIRST, false, new SeFilter[] {filter});
			}
			// TODO where clause
	...
	final SeRow row) {
		...
		if (object != null) {
			...
			for (int columnIndex = 0; columnIndex < attributes.size(); columnIndex++) {
				try {
					final SeColumnDefinition columnDefinition = row
			...
			case SeShape.TYPE_MULTI_POINT:
				...
				for (int partIndex = 0; partIndex < numParts; partIndex++) {
					...
					for (int ringIndex = 0; ringIndex < numRings; ringIndex++) {
						final CoordinatesList coordinates = getCoordinates(
								shape, allCoordinates, partIndex, ringIndex,
								numAxis);
				...
				for (int partIndex = 0; partIndex < numParts; partIndex++) {
					...
					for (int ringIndex = 0; ringIndex < numRings; ringIndex++) {
						final CoordinatesList coordinates = getCoordinates(
								shape, allCoordinates, partIndex, ringIndex,
								numAxis);
						final LineString line = geometryFactory

...
public class ArcSdeStGeometryFieldAdder extends JdbcFieldAdder {
  ...
  public FieldDefinition addField(final AbstractJdbcRecordStore recordStore,
    ...
    final int axisCount = JdbcFieldAdder.getIntegerColumnProperty(schema,
      typePath, columnName, JdbcFieldAdder.AXIS_COUNT);
    if (axisCount == -1) {
    ...
    final FieldDefinition fieldDefinition = new ArcSdeStGeometryFieldDefinition(
      dbName, name, dataType, required, description, null, spatialReference,
      axisCount);

    recordDefinition.addField(fieldDefinition);
    fieldDefinition.setProperty(FieldProperties.GEOMETRY_FACTORY,
      geometryFactory);

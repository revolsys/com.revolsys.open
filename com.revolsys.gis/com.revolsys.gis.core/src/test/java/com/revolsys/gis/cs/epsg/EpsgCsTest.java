...
public class EpsgCsTest extends TestCase {
  ...
  public void testUTransverseMeractor() {
    final ProjectedCoordinateSystem cs = (ProjectedCoordinateSystem)EpsgCoordinateSystems.getCoordinateSystem(26910);
  ...
  public void testWktRead()
    ...
    for (final Integer crsId : EpsgCoordinateSystems.getCoordinateSystemsById()
      .keySet()) {
      final CoordinateSystem cs = EpsgCoordinateSystems.getCoordinateSystem(crsId);
      if (cs instanceof ProjectedCoordinateSystem) {
        ...
        if (!projCs.isDeprecated()) {
          ...
          final CoordinatesOperation inverseOperation = ProjectionFactory.getCoordinatesOperation(
            projCs, geoCs);
          final CoordinatesOperation forwardOperation = ProjectionFactory.getCoordinatesOperation(
            geoCs, projCs);
  ...
  public void xtestGetCoordinateSystem()
    throws Exception {
    final ProjectedCoordinateSystem bcAlbers = (ProjectedCoordinateSystem)EpsgCoordinateSystems.getCoordinateSystem(3005);
    final ProjectedCoordinateSystem utm = (ProjectedCoordinateSystem)EpsgCoordinateSystems.getCoordinateSystem(26910);

    final GeographicCoordinateSystem gcs1 = bcAlbers.getGeographicCoordinateSystem();
    final GeographicCoordinateSystem gcs2 = utm.getGeographicCoordinateSystem();
    System.out.println(gcs1.equals(gcs2));
  }

  public void xtestTransverseMeractor() {
    final ProjectedCoordinateSystem cs = (ProjectedCoordinateSystem)EpsgCoordinateSystems.getCoordinateSystem(27700);
    final TransverseMercator proj = new TransverseMercator(cs);

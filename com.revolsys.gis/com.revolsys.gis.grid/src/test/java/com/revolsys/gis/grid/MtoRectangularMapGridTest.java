...
public class MtoRectangularMapGridTest extends Nts50000RectangularMapGridTest {
  ...
  protected void doTestMtoByName(
  ...
  protected void doTestNts50000(
    ...
    if (baseTileName.length() == 5) {
      baseTileName = baseTileName.substring(0, 4) + "0"
        + baseTileName.substring(4);
    }
    for (char letter = 'A'; letter <= 'L'; letter++) {
      for (int number = 1; number <= 100; number++) {
        for (char letterQuarter = 'a'; letterQuarter <= 'd'; letterQuarter++) {
          ...
          lon -= GridUtil.getLetter4Col(letterQuarter)
            * MtoConstants.WIDTH_QUARTER;

          double lat = parentLat;
          lat += GridUtil.getLetter16Row(letter) * MtoConstants.HEIGHT_TWELTH;
          lat += GridUtil.getNumberRow100(number)
            * MtoConstants.HEIGHT_HUNDRETH;
          lat += GridUtil.getLetter4Row(letterQuarter)
            * MtoConstants.HEIGHT_QUARTER;

          final String tileName = baseTileName + letter
            + GridUtil.formatSheetNumber100(number) + letterQuarter;

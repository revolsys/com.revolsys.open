...
public final class EdmDecimal extends SingletonPrimitiveType {
  ...
  private static boolean validatePrecisionAndScale(final String value, final Integer precision,
    ...
    if (matcher.group(3) != null) {
      final String plainValue = new BigDecimal(value).toPlainString();
  ...
  public boolean isCompatible(final EdmPrimitiveType primitiveType) {
    return primitiveType instanceof EdmByte || primitiveType instanceof EdmSByte
      ...
      || primitiveType instanceof EdmInt64 || primitiveType instanceof EdmSingle
      || primitiveType instanceof EdmDouble || primitiveType instanceof EdmDecimal;
  }
  ...
  @Override
  public boolean validateDecimals(final String value, final Boolean isNullable,
  ...
  private boolean validatePrecisionAndScale(final String value, final Integer precision,
    final String scale) {
    Matcher matcher = PATTERN.matcher(value);
    matcher.matches();
    if (matcher.group(3) != null) {
    ...
    try {
      final int scaleValue = scale == null ? 0 : Integer.parseInt(scale);
      return (precision == null
        || significantIntegerDigits >= 0 && significantIntegerDigits <= precision - scaleValue)
        && decimals >= 0 && decimals <= scaleValue;
    } catch (final NumberFormatException e) {
      final String scaleValue = scale == null ? String.valueOf(0) : scale;
      if (scaleValue.equals("variable")) {
        return (precision == null
          || significantIntegerDigits >= 0 && significantIntegerDigits <= precision - decimals)
          && decimals >= 0 && decimals <= (precision == null ? 0 : precision);
      } else if (scaleValue.equals("floating")) {
        final Matcher matcher1 = PATTERN.matcher(value);
        matcher1.matches();
        ...
        if (matcher1.group(3) != null) {
          ...
          if (exponents < -95 || exponents > 96) {
            if (String.valueOf(exponents).startsWith("-")) {
              significantIntegerDigits += Integer
                .parseInt(String.valueOf(exponents + 95).substring(1));
              exponents = -95;
            }
          }

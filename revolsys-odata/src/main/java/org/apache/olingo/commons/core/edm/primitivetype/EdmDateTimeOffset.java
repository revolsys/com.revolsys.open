...
public final class EdmDateTimeOffset extends SingletonPrimitiveType {
  ...
  private static <T> T convertZonedDateTime(final ZonedDateTime zdt, final Class<T> returnType) {
  ...
  private static <T> ZonedDateTime createZonedDateTime(final T value)
    ...
    if (value instanceof GregorianCalendar) {
      ...
      final ZonedDateTime zdt = calendar.toZonedDateTime();
      final ZoneId normalizedZoneId = calendar.getTimeZone().toZoneId().normalized();
  ...
  private static String format(final LocalDateTime dateTime, final ZoneOffset offset,
    final int nanos) {
  ...
  private static String removeTrailingZeros(final String str) {
  ...
  protected <T> T internalValueOfString(final String value, final Boolean isNullable,
    ...
    try {
      final ZonedDateTime zdt = parseZonedDateTime(value);
  ...
  protected <T> String internalValueToString(final T value, final Boolean isNullable,
    final Integer maxLength, final Integer precision, final Integer scale, final Boolean isUnicode)
    throws EdmPrimitiveTypeException {
    final ZonedDateTime zdt = createZonedDateTime(value);

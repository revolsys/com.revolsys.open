...
public class Encoder {
  ...
  private String encodeInternal(final String input) {
    ...
    try {
      for (final byte utf8Byte : input.getBytes("UTF-8")) {
        ...
        if (isUnreserved(character)) {
          ...
          resultStr.append(hex[256 + utf8Byte]); // index adjusted for the usage
                                                 // of signed bytes
        }
      }
    } catch (final UnsupportedEncodingException e) { // should never happen;
  ...
  private boolean isUnencoded(final char character) {
    return this.unencoded.indexOf(character) >= 0;
  }
}

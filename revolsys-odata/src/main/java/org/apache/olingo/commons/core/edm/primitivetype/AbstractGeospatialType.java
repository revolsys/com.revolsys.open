...
public abstract class AbstractGeospatialType<T extends Geospatial> extends SingletonPrimitiveType {
  ...
  protected AbstractGeospatialType(final Class<T> reference, final Dimension dimension,
    final Type type) {
  ...
  private StringBuilder appendPoints(final ComposedGeospatial<Point> points,
    final Boolean isNullable, final Integer maxLength, final Integer precision, final Integer scale,
    final Boolean isUnicode, final StringBuilder result) throws EdmPrimitiveTypeException {
  ...
  private LineString newLineString(final SRID srid, final String lineString,
    final Boolean isNullable, final Integer maxLength, final Integer precision, final Integer scale,
  ...
  private Point newPoint(final SRID srid, final String point, final Boolean isNullable,
    ...
    result.setX(EdmDouble.getInstance()
      .valueOfString(pointCoo.get(0), isNullable, maxLength, precision, scale, isUnicode,
        Double.class));
    result.setY(EdmDouble.getInstance()
      .valueOfString(pointCoo.get(1), isNullable, maxLength, precision, scale, isUnicode,
        Double.class));
  ...
  private Polygon newPolygon(final SRID srid, final String polygon, final Boolean isNullable,
    ...
    for (int i = 0; i < first.length - 1; i++) {
      ...
      interiorRings.add(new LineString(this.dimension, srid, interior));
    }
    final List<Point> exterior = new ArrayList<>();
    for (final String pointCoo : split(
      first[first.length - 1].substring(0, first[first.length - 1].length() - 1), ',')) {
      exterior.add(newPoint(null, pointCoo, isNullable, maxLength, precision, scale, isUnicode));
    }

    return new Polygon(this.dimension, srid, interiorRings,
      new LineString(this.dimension, srid, exterior));
  }

  private String point(final Point point, final Boolean isNullable, final Integer maxLength,
    final Integer precision, final Integer scale, final Boolean isUnicode)
    throws EdmPrimitiveTypeException {

    return new StringBuilder()
      .append(EdmDouble.getInstance()
        .valueToString(point.getX(), isNullable, maxLength, precision, scale, isUnicode))
      .append(' ')
      .append(EdmDouble.getInstance()
        .valueToString(point.getY(), isNullable, maxLength, precision, scale, isUnicode))
      .toString();
  }

  private String polygon(final Polygon polygon, final Boolean isNullable, final Integer maxLength,
    final Integer precision, final Integer scale, final Boolean isUnicode)
    throws EdmPrimitiveTypeException {
    ...
    for (int i = 0; i < polygon.getNumberOfInteriorRings(); i++) {
      result.append('(');
      appendPoints(polygon.getInterior(i), isNullable, maxLength, precision, scale, isUnicode,
        result);
      result.append("),");
    }
  ...
  protected GeospatialCollection stringToCollection(final String value, final Boolean isNullable,
    ...
    switch (Geospatial.Type.valueOf(matcher.group(3).toUpperCase())) {
      case POINT:
        item = newPoint(SRID.valueOf(matcher.group(2)), matcher.group(4), isNullable, maxLength,
      ...
      case MULTIPOINT:
        ...
        for (final String pointCoo : split(matcher.group(4), ',')) {
          points.add(newPoint(null, pointCoo.substring(1, pointCoo.length() - 1), isNullable,
      ...
      case LINESTRING:
        item = newLineString(SRID.valueOf(matcher.group(2)), matcher.group(4), isNullable,
          maxLength, precision, scale, isUnicode);
      break;
      ...
      case MULTILINESTRING:
        ...
        for (final String coo : split(matcher.group(4), ',')) {
          lineStrings.add(newLineString(null, coo.substring(1, coo.length() - 1), isNullable,
            maxLength, precision, scale, isUnicode));
        }
      ...
      case POLYGON:
        item = newPolygon(SRID.valueOf(matcher.group(2)), matcher.group(4), isNullable, maxLength,
          precision, scale, isUnicode);
      break;
      ...
      case MULTIPOLYGON:
        ...
        for (final String coo : split(matcher.group(4), ',')) {
          polygons.add(newPolygon(null, coo.substring(1, coo.length() - 1), isNullable, maxLength,
            precision, scale, isUnicode));
        }
    ...
    return new GeospatialCollection(this.dimension, SRID.valueOf(matcher.group(2)),
      Collections.<Geospatial> singletonList(item));
  }

  protected LineString stringToLineString(final String value, final Boolean isNullable,
    final Integer maxLength, final Integer precision, final Integer scale, final Boolean isUnicode)
    throws EdmPrimitiveTypeException {

    final Matcher matcher = getMatcher(PATTERN, value);

    return newLineString(SRID.valueOf(matcher.group(2)), matcher.group(4), isNullable, maxLength,
      precision, scale, isUnicode);
  }
  ...
  protected MultiLineString stringToMultiLineString(final String value, final Boolean isNullable,
    ...
    final List<LineString> lineStrings = new ArrayList<>();
    for (final String coo : matcher.group(4).contains("),(") ? matcher.group(4).split("\\),\\(")
      : new String[] {
        matcher.group(4)
      }) {
      ...
      lineStrings
        .add(newLineString(null, lineString, isNullable, maxLength, precision, scale, isUnicode));
    }

    return new MultiLineString(this.dimension, SRID.valueOf(matcher.group(2)), lineStrings);
  }

  protected MultiPoint stringToMultiPoint(final String value, final Boolean isNullable,
    final Integer maxLength, final Integer precision, final Integer scale, final Boolean isUnicode)
    throws EdmPrimitiveTypeException {
    ...
    final List<Point> points = new ArrayList<>();
    for (final String pointCoo : split(matcher.group(4), ',')) {
      points.add(newPoint(null, pointCoo.substring(1, pointCoo.length() - 1), isNullable, maxLength,
        precision, scale, isUnicode));
    }
  ...
  protected MultiPolygon stringToMultiPolygon(final String value, final Boolean isNullable,
    ...
    final List<Polygon> polygons = new ArrayList<>();
    for (final String coo : matcher.group(4).contains(")),((")
  ...
  protected Point stringToPoint(final String value, final Boolean isNullable,
    final Integer maxLength, final Integer precision, final Integer scale, final Boolean isUnicode)
    throws EdmPrimitiveTypeException {

    final Matcher matcher = getMatcher(PATTERN, value);

    return newPoint(SRID.valueOf(matcher.group(2)), matcher.group(4), isNullable, maxLength,
      precision, scale, isUnicode);
  }

  protected Polygon stringToPolygon(final String value, final Boolean isNullable,
    final Integer maxLength, final Integer precision, final Integer scale, final Boolean isUnicode)
    throws EdmPrimitiveTypeException {

    final Matcher matcher = getMatcher(PATTERN, value);

    return newPolygon(SRID.valueOf(matcher.group(2)), matcher.group(4), isNullable, maxLength,
      precision, scale, isUnicode);
  }

  protected String toString(final GeospatialCollection collection, final Boolean isNullable,
    final Integer maxLength, final Integer precision, final Integer scale, final Boolean isUnicode)
    throws EdmPrimitiveTypeException {
    ...
    final StringBuilder result = toStringBuilder(collection.getSrid()).append("Collection(");
    ...
    if (collection.iterator().hasNext()) {
      ...
      switch (item.getEdmPrimitiveTypeKind()) {
        ...
        case GeometryLineString:
          appendPoints((LineString)item, isNullable, maxLength, precision, scale, isUnicode,
            result);
        break;

        case GeographyMultiLineString:
        case GeometryMultiLineString:
          for (final Iterator<LineString> itor = ((MultiLineString)item).iterator(); itor
            .hasNext();) {
            result.append('(');
  ...
  protected String toString(final LineString lineString, final Boolean isNullable,
    final Integer maxLength, final Integer precision, final Integer scale, final Boolean isUnicode)
    throws EdmPrimitiveTypeException {
    ...
    final StringBuilder builder = toStringBuilder(lineString.getSrid())
      .append(this.reference.getSimpleName())
      .append('(');
  ...
  protected String toString(final MultiLineString multiLineString, final Boolean isNullable,
    final Integer maxLength, final Integer precision, final Integer scale, final Boolean isUnicode)
    throws EdmPrimitiveTypeException {
  ...
  protected String toString(final MultiPoint multiPoint, final Boolean isNullable,
    final Integer maxLength, final Integer precision, final Integer scale, final Boolean isUnicode)
    throws EdmPrimitiveTypeException {
  ...
  protected String toString(final MultiPolygon multiPolygon, final Boolean isNullable,
    final Integer maxLength, final Integer precision, final Integer scale, final Boolean isUnicode)
    throws EdmPrimitiveTypeException {
  ...
  protected String toString(final Point point, final Boolean isNullable, final Integer maxLength,
    ...
    return toStringBuilder(point.getSrid()).append(this.reference.getSimpleName())
      .append('(')
  ...
  protected String toString(final Polygon polygon, final Boolean isNullable,
    final Integer maxLength, final Integer precision, final Integer scale, final Boolean isUnicode)
    throws EdmPrimitiveTypeException {
    ...
    return toStringBuilder(polygon.getSrid()).append(this.reference.getSimpleName())
      .append('(')
  ...
  private StringBuilder toStringBuilder(final SRID srid) {
    return new StringBuilder(this.dimension.name().toLowerCase()).append('\'')
      .append("SRID=")
      .append(srid)
      .append(';');
  }

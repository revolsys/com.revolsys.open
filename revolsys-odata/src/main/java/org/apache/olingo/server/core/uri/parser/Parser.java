...
public class Parser {
  ...
  private void checkOptionEOF(final UriTokenizer tokenizer, final String optionName,
    final String optionValue) throws UriParserException {
  ...
  private void parseApplyOption(final ApplyOption applyOption, final EdmType contextType,
  ...
  private void parseExpandOption(final ExpandOption expandOption, final EdmType contextType,
    final boolean isAll, final List<String> entitySetNames,
  ...
  private QueryOption parseOption(final String optionName, final String optionValue)
    ...
    if (optionName.startsWith(DOLLAR)) {
      ...
      switch (kind) {
        case SEARCH:
          final SearchOption searchOption = new SearchParser().parse(optionValue);
          final SearchOptionImpl tmp = new SearchOptionImpl();
        ...
        break;
        case FORMAT:
          if (optionValue.equalsIgnoreCase(JSON) || optionValue.equalsIgnoreCase(XML)
            || optionValue.equalsIgnoreCase(ATOM) || isFormatSyntaxValid(optionValue)) {
            systemOption = new FormatOptionImpl().setFormat(optionValue);
          } else {
            throw new UriParserSyntaxException("Illegal value of $format option!",
              UriParserSyntaxException.MessageKeys.WRONG_VALUE_FOR_SYSTEM_QUERY_OPTION_FORMAT,
              optionValue);
          }
        break;
        case ID:
        ...
        case LEVELS:
          throw new UriParserSyntaxException(
            "System query option '$levels' is allowed only inside '$expand'!",
      ...
      // Aliases can only be parsed in the context of their usage, so the value
      // is not checked here.
      return new AliasQueryOptionImpl().setName(optionName)
        .setText(NULL.equals(optionValue) ? null : optionValue);

    } else {
      // The option is a custom query option; the caller can re-use its query
      // option.
      return null;
    }
  }

  private void parseOrderByOption(final OrderByOption orderByOption, final EdmType contextType,
    final List<String> entitySetNames, final Map<String, AliasQueryOption> aliases)
    throws UriParserException, UriValidationException {
  ...
  private void parseSelectOption(final SelectOption selectOption, final EdmType contextType,
  ...
  public UriInfo parseUri(final String path, final String query, final String fragment,
    String baseUri) throws UriParserException, UriValidationException {

    UriInfoImpl contextUriInfo = new UriInfoImpl();

    // Read the query options (system and custom options).
    // This is done before parsing the resource path because the aliases have to
    // be available there.
    // System query options that can only be parsed with context from the
    // resource path will be post-processed later.
    final List<QueryOption> options = query == null ? Collections.<QueryOption> emptyList()
    ...
    for (final QueryOption option : options) {
      ...
      try {
        ...
        throw new UriParserSyntaxException(
          parsedOption instanceof SystemQueryOption ? "Double system query option!"
    ...
    if (firstSegment.isEmpty()) {
      ...
      if (null != contextUriInfo.getIdOption()) {
         ...
         */
        throw new UriParserSyntaxException(
          "The entity-id MUST be specified using the system query option $id",
      ...
      for (final String pathSegment : pathSegmentsDecoded) {
        ...
        if (pathSegment.startsWith(ENTITY)) {
          ...
          if (segment != null) {
            if (segment instanceof UriResourceCount || segment instanceof UriResourceRef
              ...
              throw new UriParserSemanticException(
                "First resource-path segment must not be namespace-qualified.",
    ...
    // Post-process system query options that need context information from the
    // resource path.

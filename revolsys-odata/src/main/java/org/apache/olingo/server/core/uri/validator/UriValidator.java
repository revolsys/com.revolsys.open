...
public class UriValidator {
  ...
  private enum UriType {
    all(0), batch(1), crossjoin(2), entityId(3), metadata(4), service(5), entitySet(
      6), entitySetCount(7), entity(8), mediaStream(9), references(10), reference(
        11), propertyComplex(12), propertyComplexCollection(13), propertyComplexCollectionCount(
          14), propertyPrimitive(15), propertyPrimitiveCollection(
            16), propertyPrimitiveCollectionCount(17), propertyPrimitiveValue(18), none(19);
  ...
  private static final boolean[][] decisionMatrix = {
    /*
     * 0-FILTER 1-FORMAT 2-EXPAND 3-ID 4-COUNT 5-ORDERBY 6-SEARCH 7-SELECT
     * 8-SKIP 9-SKIPTOKEN 10-TOP 11-APPLY 12-DELTATOKEN
     */
    /* all 0 */ {
  ...
  static {
    final Map<SystemQueryOptionKind, Integer> temp = new EnumMap<>(SystemQueryOptionKind.class);
  ...
  private boolean checkIfSelectOrExpand(final List<SystemQueryOption> options,
    final HttpMethod httpMethod) {
    boolean isSelectOrExpand = false;
    for (final SystemQueryOption queryOption : options) {
  ...
  private UriType getUriTypeForAction(final UriResource lastPathSegment)
    ...
    switch (rt.getType().getKind()) {
      ...
      case ENUM:
      case DEFINITION:
        uriType = rt.isCollection() ? UriType.propertyPrimitiveCollection
          : UriType.propertyPrimitive;
      break;
      case COMPLEX:
  ...
  private UriType getUriTypeForComplexProperty(final UriResource lastPathSegment)
    throws UriValidationException {
    return isCollection(lastPathSegment) ? UriType.propertyComplexCollection
      : UriType.propertyComplex;
  }

  private UriType getUriTypeForCount(final UriResource secondLastPathSegment)
    throws UriValidationException {
    UriType uriType;
  ...
  private UriType getUriTypeForEntitySet(final UriResource lastPathSegment)
    throws UriValidationException {
  ...
  private UriType getUriTypeForPrimitiveProperty(final UriResource lastPathSegment)
    throws UriValidationException {
    return isCollection(lastPathSegment) ? UriType.propertyPrimitiveCollection
      : UriType.propertyPrimitive;
  }

  private UriType getUriTypeForRef(final UriResource secondLastPathSegment)
    throws UriValidationException {
   ...
   */
  private UriType getUriTypeForResource(final List<UriResource> segments)
    throws UriValidationException {
  ...
  private UriType getUriTypeForValue(final UriResource secondLastPathSegment)
    throws UriValidationException {
    UriType uriType;
    switch (secondLastPathSegment.getKind()) {
      case primitiveProperty:
  ...
  public void validate(final UriInfo uriInfo, final HttpMethod httpMethod)
    throws UriValidationException {
    final UriType uriType = getUriType(uriInfo);
    if (HttpMethod.GET == httpMethod) {
      validateReadQueryOptions(uriType, uriInfo.getSystemQueryOptions());
    } else {
      validateNonReadQueryOptions(uriType, isAction(uriInfo), uriInfo.getSystemQueryOptions(),
        httpMethod);
  ...
  private void validateNonReadQueryOptions(final UriType uriType, final boolean isAction,
    final List<SystemQueryOption> options, final HttpMethod httpMethod)
    throws UriValidationException {
    if (httpMethod == HttpMethod.POST && isAction) {
      ...
      // system query options that a service MAY support, following the same
      // rules as GET requests.
      validateReadQueryOptions(uriType, options);

    } else if (httpMethod == HttpMethod.DELETE && uriType == UriType.references) {
      // Only $id is allowed as SystemQueryOption for DELETE on a reference
      // collection.
  ...
  private void validatePropertyOperations(final UriInfo uriInfo, final HttpMethod method)
    ...
    if (last != null && (last.getKind() == UriResourceKind.primitiveProperty
      || last.getKind() == UriResourceKind.complexProperty
      || last.getKind() == UriResourceKind.value && previous != null
        && previous.getKind() == UriResourceKind.primitiveProperty)) {
      final EdmProperty property = ((UriResourceProperty)(last.getKind() == UriResourceKind.value
        ? previous
        : last)).getProperty();
  ...
  private void validateReadQueryOptions(final UriType uriType,
    final List<SystemQueryOption> options) throws UriValidationException {
    for (final SystemQueryOption option : options) {

...
public class BatchTransformatorCommon {
  ...
  public static int getContentLength(final Header headers) throws BatchDeserializerException {
    ...
    if (contentLengthField != null && contentLengthField.getValues().size() == 1) {
      try {
        ...
        throw new BatchDeserializerException("Invalid content length", e,
          MessageKeys.INVALID_CONTENT_LENGTH, Integer.toString(contentLengthField.getLineNumber()));
      }
    }
  ...
  public static void validateContentTransferEncoding(final Header headers)
    ...
    final HeaderField contentTransferField = headers
      .getHeaderField(BatchParserCommon.CONTENT_TRANSFER_ENCODING);

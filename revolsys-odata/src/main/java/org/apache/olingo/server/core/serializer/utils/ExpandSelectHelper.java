...
public abstract class ExpandSelectHelper {
  ...
  private static Set<List<String>> extractPathsFromResourceParts(
    final Set<List<String>> selectedPaths, final List<UriResource> parts, final List<String> path) {
    if (parts.size() > 1) {
  ...
  public static Set<List<String>> getExpandedItemsPath(final ExpandOption expand) {
    final Set<List<String>> expandPaths = new HashSet<>();
    if (expand != null) {
      final List<ExpandItem> expandItems = expand.getExpandItems();
  ...
  public static ExpandItem getExpandItemBasedOnType(final List<ExpandItem> expandItems,
    final String propertyName, final EdmStructuredType type, final String resourceName) {
  ...
  public static Set<List<String>> getReducedExpandItemsPaths(
    final Set<List<String>> expandItemsPaths, final String propertyName) {
    final Set<List<String>> reducedPaths = new HashSet<>();
   ...
   */
  public static Set<List<String>> getSelectedPathsWithTypeCasts(final List<SelectItem> selectItems,
    final String propertyName) {
    final Set<List<String>> selectedPaths = new HashSet<>();
    for (final SelectItem item : selectItems) {
      final List<UriResource> parts = item.getResourcePath().getUriResourceParts();
   ...
   * @return
   */
  private static boolean isFoundExpandItem(final EdmStructuredType type, boolean matched,

...
public class AsyncResponseSerializer {
  ...
  private void append(final String value, final ByteArrayOutputStream buffer) throws IOException {
    try {
      ...
      throw new IOException(
        "Default header charset with name '" + HEADER_CHARSET_NAME + "' is not available.", e);
    }
  }

  private void appendBody(final ODataResponse response, final ByteArrayOutputStream buffer)
    throws IOException {
    final InputStream input = response.getContent();
    if (input != null) {
  ...
  private void appendHeader(final String name, final List<String> values,
  ...
  public InputStream serialize(final ODataResponse response) throws SerializerException {
    try {
      ...
      throw new SerializerException(
        "Exception occurred during serialization of asynchronous response.", e,

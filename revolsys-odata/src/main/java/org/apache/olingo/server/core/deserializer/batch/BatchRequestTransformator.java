...
public class BatchRequestTransformator {
  ...
  private ODataRequest createRequest(final BatchQueryOperation operation, final String baseUri,
    ...
    final HttpRequestStatusLine statusLine = new HttpRequestStatusLine(
      operation.getHttpStatusLine(), baseUri, this.rawServiceResolutionUri);
    ...
    final Charset charset = getCharset(operation);
    final InputStream bodyStream = getBodyStream(operation, statusLine, charset);
  ...
  private HeaderField getContentId(final BatchQueryOperation queryOperation)
    throws BatchDeserializerException {
    final HeaderField contentIdHeader = queryOperation.getHeaders()
    ...
    if (contentIdHeader != null) {
      if (contentIdHeader.getValues().size() == 1) {
        ...
        throw new BatchDeserializerException("Invalid Content-ID header",
          MessageKeys.INVALID_CONTENT_ID, Integer.toString(contentIdHeader.getLineNumber()));
      }
    }
  ...
  private void handleContentId(final BatchQueryOperation changeRequestPart,
    final BatchQueryOperation request) throws BatchDeserializerException {
  ...
  private ODataRequest processQueryOperation(final BatchBodyPart bodyPart, final String baseUri,
    ...
    if (bodyPart.isChangeSet()) {
      final BatchQueryOperation encapsulatedQueryOperation = ((BatchChangeSetPart)queryOperation)
        .getRequest();
  ...
  public List<BatchRequestPart> transform(final BatchBodyPart bodyPart)
    throws BatchDeserializerException {
    ...
    for (final BatchQueryOperation queryOperation : bodyPart.getRequests()) {
      requests.add(processQueryOperation(bodyPart, this.baseUri, queryOperation));
    }
  ...
  private void validateForbiddenHeader(final BatchQueryOperation operation)
    ...
    if (header.exists(HttpHeader.WWW_AUTHENTICATE) || header.exists(HttpHeader.AUTHORIZATION)
      ...
      || header.exists(HttpHeader.MAX_FORWARDS) || header.exists(HttpHeader.RANGE)
      || header.exists(HttpHeader.TE)) {
      throw new BatchDeserializerException("Forbidden header", MessageKeys.FORBIDDEN_HEADER,
        Integer.toString(header.getLineNumber()));
    }
  }

  private void validateHeaders(final Header headers, final boolean isChangeSet)
    throws BatchDeserializerException {
    if (isChangeSet) {

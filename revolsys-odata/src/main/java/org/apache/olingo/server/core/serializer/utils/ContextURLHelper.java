...
public final class ContextURLHelper {
  ...
  private static void appendExpandedProperty(final StringBuilder result, final String propertyName)
  ...
  public static String buildKeyPredicate(final List<UriParameter> keys) throws SerializerException {
    if (keys == null || keys.isEmpty()) {
      ...
      for (final UriParameter key : keys) {
        ...
        result.append(Encoder.encode(key.getName()))
          .append('=')
          .append(Encoder.encode(key.getText()));
      }
   ...
   */
  public static String buildSelectList(final EdmStructuredType type, final ExpandOption expand,
    ...
    if (ExpandSelectHelper.hasExpand(expand)
      && !(null != ExpandSelectHelper.getExpandAll(expand))) {
   ...
   * @param propertyName
   */
  private static void constructSelectItemList(final EdmStructuredType type,
    final StringBuilder result, final List<SelectItem> selectItems,
    final Set<String> selectedPropertyNames, final String propertyName) {
    if (selectedPropertyNames.contains(propertyName)) {
      ...
      if (selectedPaths == null) {
        ...
        final List<List<String>> complexSelectedPaths = edmProperty != null
   ...
   * @param part
   */
  private static int getComplexPropertyPosition(final Set<List<String>> selectedPaths,
    final UriResourceComplexProperty part) {
  ...
  private static List<List<String>> getComplexSelectedPaths(
    ...
    if (selectedPaths == null) {
      ...
      for (final String complexPropertyName : type.getPropertyNames()) {
        if (ExpandSelectHelper.isSelected(selectedPaths, complexPropertyName)) {
          final List<List<String>> complexSelectedPaths = getComplexSelectedPaths(
    ...
    for (final SelectItem item : selectItems) {
      ...
      for (final UriResource part : parts) {
        if (part instanceof UriResourceComplexProperty
          && ((UriResourceComplexProperty)part).getProperty()
            .getName()
   ...
   * @return
   */
  private static EdmStructuredType getTypeFromSelectItems(final List<SelectItem> selectItems,
  ...
  private static void handleExpand(final EdmStructuredType type, final ExpandOption expand,
    final StringBuilder result) throws SerializerException {
    final Set<String> expandedPropertyNames = ExpandSelectHelper
      .getExpandedPropertyNames(expand.getExpandItems());
    for (final String propertyName : type.getNavigationPropertyNames()) {
      if (expandedPropertyNames.contains(propertyName)) {

        final ExpandItem expandItem = ExpandSelectHelper.getExpandItem(expand.getExpandItems(),
          propertyName);
        if (ExpandSelectHelper.hasExpand(expandItem.getExpandOption())
          ...
          if (innerSelectList != null) {
            ...
            result.append(Encoder.encode(propertyName))
              .append('(')
              .append(innerSelectList)
              .append(')');
          }
        } else {
          final List<UriResource> resourceParts = expandItem.getResourcePath()
            .getUriResourceParts();
  ...
  private static void handleExpandAll(final EdmStructuredType type, final ExpandOption expand,
    final StringBuilder result) throws SerializerException {
    for (final String propertyName : type.getNavigationPropertyNames()) {
      appendExpandedProperty(result, propertyName);
    }
  ...
  private static void handleSelect(EdmStructuredType type, final SelectOption select,
    ...
    if (ExpandSelectHelper.isAll(select)) {
      ...
      final Set<String> selectedPropertyNames = ExpandSelectHelper
        .getSelectedPropertyNames(selectItems);

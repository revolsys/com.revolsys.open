...
public class BatchParserCommon {
  ...
  // "." / "/" / ":" / "=" / "?"
  // The first alternative is for the case that only characters are used that
  // don't need quoting.
  ...
  private static final Pattern PATTERN_HEADER_LINE = Pattern
    .compile("((?:\\w|[!#$%\\&'*+\\-.^`|~])+):\\s?(.*)\\s*");
  ...
  public static InputStream convertLineListToInputStream(final List<Line> list,
    final Charset charset, final int length) {
  ...
  private static String lineListToString(final List<Line> messageList) {
    ...
    for (final Line currentLine : messageList) {
   ...
   * @throws BatchDeserializerException
   */
  public static ContentType parseContentType(final String contentType, final ContentType expected,
    final int line) throws BatchDeserializerException {
    if (contentType == null) {
    ...
    if (type.isCompatible(expected)) {
      ...
      throw new BatchDeserializerException("Content type is not the expected content type",
        BatchDeserializerException.MessageKeys.UNEXPECTED_CONTENT_TYPE, Integer.toString(line),
        expected.toContentTypeString(), type.toContentTypeString());
    }
  }

  public static Line removeEndingCRLF(final Line line) {
    final Matcher matcher = PATTERN_LAST_CRLF.matcher(line.toString());
    if (matcher.matches()) {
  ...
  private static void removeEndingCRLFFromList(final List<Line> list) {
    if (!list.isEmpty()) {
      final Line lastLine = list.remove(list.size() - 1);
      list.add(removeEndingCRLF(lastLine));
    }
  }

  public static List<List<Line>> splitMessageByBoundary(final List<Line> message,
    final String boundary) throws BatchDeserializerException {

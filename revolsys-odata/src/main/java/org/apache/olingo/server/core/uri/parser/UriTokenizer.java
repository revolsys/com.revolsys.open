...
public class UriTokenizer {
  ...
  public enum TokenKind {
    ...
    REF, VALUE, COUNT, CROSSJOIN, ROOT, IT,

    APPLY, // for the aggregation extension
    EXPAND, FILTER, LEVELS, ORDERBY, SEARCH, SELECT, SKIP, TOP,

    ANY, ALL,

    OPEN, CLOSE, COMMA, SEMI, COLON, DOT, SLASH, EQ, STAR, PLUS,
    ...
    ODataIdentifier, QualifiedName, ParameterAliasName,

    BooleanValue, StringValue, IntegerValue, GuidValue, DateValue, DateTimeOffsetValue, TimeOfDayValue, DecimalValue, DoubleValue, DurationValue, BinaryValue, EnumValue,

    GeographyPoint, GeometryPoint, GeographyLineString, GeometryLineString, GeographyPolygon, GeometryPolygon, GeographyMultiPoint, GeometryMultiPoint, GeographyMultiLineString, GeometryMultiLineString, GeographyMultiPolygon, GeometryMultiPolygon, GeographyCollection, GeometryCollection,

    jsonArrayOrObject,

    Word, Phrase,

    OrOperatorSearch, AndOperatorSearch, NotOperatorSearch,

    OrOperator, AndOperator, EqualsOperator, NotEqualsOperator, GreaterThanOperator, GreaterThanOrEqualsOperator, LessThanOperator, LessThanOrEqualsOperator, HasOperator, InOperator, AddOperator, SubOperator, MulOperator, DivOperator, ModOperator, MinusOperator, NotOperator,
  ...
  public boolean next(final TokenKind allowedTokenKind) {
    ...
    boolean found = false;
    final int previousIndex = this.index;
    switch (allowedTokenKind) {
      ...
      break;
      case MinusOperator:
        // To avoid unnecessary minus operators for negative numbers, we have to
        // check what follows the minus sign.
  ...
  private boolean nextAndOperatorSearch() {
    if (nextWhitespace()) {
      final int lastGoodIndex = this.index;
  ...
  private boolean nextBase64() {
    return nextCharacterRange('0', '9') || nextCharacterRange('A', 'Z')
      || nextCharacterRange('a', 'z') || nextCharacter('-') || nextCharacter('_');
  }
    ...
    if (nextConstantIgnoreCase("binary") && nextCharacter('\'')) {
      ...
      if (nextBase64() && nextBase64()
        && (nextCharacter('A') || nextCharacter('E') || nextCharacter('I') || nextCharacter('M')
          ...
          || nextCharacter('w') || nextCharacter('0') || nextCharacter('4')
          || nextCharacter('8'))) {
        nextCharacter('=');
      } else {
        this.index = lastGoodIndex;
  ...
  private boolean nextCharacter(final char character) {
    if (this.index < this.parseString.length()
      && this.parseString.charAt(this.index) == character) {
      this.index++;
      return true;
    } else {
      return false;
  ...
  private boolean nextCharacterRange(final char from, final char to) {
    if (this.index < this.parseString.length()) {
      final char code = this.parseString.charAt(this.index);
  ...
  private boolean nextConstantIgnoreCase(final String constant) {
    ...
    if (this.index + length <= this.parseString.length()
      && constant.equalsIgnoreCase(this.parseString.substring(this.index, this.index + length))) {
      this.index += length;
      return true;
    } else {
      return false;
  ...
  private boolean nextDateTimeOffsetValue() {
    return nextDateValue() && (nextCharacter('T') || nextCharacter('t')) && nextTimeOfDayValue()
      && (nextCharacter('Z') || nextCharacter('z')
        || nextSign() && nextHours() && nextCharacter(':') && nextMinutesOrSeconds());
  }

  private boolean nextDateValue() {
    return nextYear() && nextCharacter('-')
  ...
  private boolean nextGuidValue() {
    return nextHexDigit() && nextHexDigit() && nextHexDigit() && nextHexDigit() && nextHexDigit()
      && nextHexDigit() && nextHexDigit() && nextHexDigit() && nextCharacter('-') && nextHexDigit()
      && nextHexDigit() && nextHexDigit() && nextHexDigit() && nextCharacter('-') && nextHexDigit()
      && nextHexDigit() && nextHexDigit() && nextHexDigit() && nextCharacter('-') && nextHexDigit()
      && nextHexDigit() && nextHexDigit() && nextHexDigit() && nextCharacter('-') && nextHexDigit()
      && nextHexDigit() && nextHexDigit() && nextHexDigit() && nextHexDigit() && nextHexDigit()
      && nextHexDigit() && nextHexDigit() && nextHexDigit() && nextHexDigit() && nextHexDigit()
      && nextHexDigit();
  }
  ...
  private boolean nextJsonString() {
    ...
    if (nextCharacter('"')) {
      do {
        if (nextCharacter('\\')) {
          if (!(nextCharacter('b') || nextCharacter('t') || nextCharacter('n') || nextCharacter('f')
            || nextCharacter('r') || nextCharacter('"') || nextCharacter('/') || nextCharacter('\\')
            || nextCharacter('u') && nextHexDigit() && nextHexDigit() && nextHexDigit()
              && nextHexDigit())) {
            this.index = lastGoodIndex;
            return false;
          }
  ...
  private boolean nextJsonValue() {
    return nextConstant("null") || nextConstant("true") || nextConstant("false")
      || nextDoubleValue() || nextDecimalValue() || nextIntegerValue(true) || nextJsonString()
  ...
  private boolean nextLineStringData(final boolean isRing) {
    ...
    if (nextCharacter('(') && nextPosition()) {
      int count = 1;
      final String firstPosition = isRing
        ? this.parseString.substring(lastGoodIndex + 1, this.index)
        : null;
      int positionStart = -1;
      while (nextCharacter(',')) {
      ...
      if (isRing) {
        final String lastPosition = this.parseString.substring(positionStart, this.index);
  ...
  private boolean nextMultiLineString() {
    if (nextConstantIgnoreCase("MultiLineString") && nextCharacter('(')
      && nextLineStringData(false)) {
      while (nextCharacter(',')) {
  ...
  private boolean nextODataIdentifier() {
    int count = 0;
    if (this.index < this.parseString.length()) {
      int code = this.parseString.codePointAt(this.index);
      if (Character.isUnicodeIdentifierStart(code) || code == '_') {
        ...
        while (this.index < this.parseString.length() && count < 128) {
          code = this.parseString.codePointAt(this.index);
  ...
  private boolean nextPosition() {
    final int lastGoodIndex = this.index;
    if ((nextDoubleValue() || nextDecimalValue() || nextIntegerValue(true)) && nextCharacter(' ')
  ...
  private boolean nextStringValue() {
    ...
    while (this.index < this.parseString.length()) {
      if (this.parseString.charAt(this.index) == '\'') {
        ...
        if (this.index + 1 < this.parseString.length()
          && this.parseString.charAt(this.index + 1) == '\'') {
          this.index++;
        } else {
          break;
        }
  ...
  private boolean nextWord() {
    int count = 0;
    while (this.index < this.parseString.length()) {
      final int code = this.parseString.codePointAt(this.index);
    ...
    final String word = this.parseString.substring(this.index - count, this.index);

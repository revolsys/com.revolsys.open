...
import com.revolsys.swing.map.layer.raster.filter.GeneralMatrix;
...
public class JaiGeoReferencedImage extends AbstractPropertyChangeObject
  ...
  private final Map<CoordinateSystem, JaiGeoReferencedImage> projectedImages = new HashMap<CoordinateSystem, JaiGeoReferencedImage>();
  ...
  public JaiGeoReferencedImage() {
  }

  public JaiGeoReferencedImage(final BoundingBox boundingBox,
  ...
  public JaiGeoReferencedImage(final Resource imageResource) {
    ...
    this.jaiImage = JAI.create("fileload", getFile().getAbsolutePath());
  ...
  @Override
  public JaiGeoReferencedImage getImage(final CoordinateSystem coordinateSystem) {
    synchronized (this.projectedImages) {
      if (coordinateSystem.equals(getCoordinateSystem())) {
        return this;
      } else {
        JaiGeoReferencedImage projectedImage = this.projectedImages.get(coordinateSystem);
  ...
  @Override
  public JaiGeoReferencedImage getImage(
    ...
    if (imageSrid > 0 && imageSrid != coordinateSystem.getId()) {
      ...
      return new JaiGeoReferencedImage(destBoundingBox, newImage);
    }
    return this;
  }

  @Override
  public JaiGeoReferencedImage getImage(final GeometryFactory geometryFactory) {
  ...
  public int getImageHeight() {
    if (this.imageHeight == -1) {
      if (this.image == null) {
        this.imageHeight = jaiImage.getHeight();
      } else {
  ...
  public int getImageWidth() {
    if (this.imageWidth == -1) {
      if (this.image == null) {
        this.imageWidth = jaiImage.getWidth();
      } else {
  ...
  public synchronized BufferedImage getWarpedImage() {
    if (this.image == null || this.boundingBox.isEmpty()) {
      return this.image;
    } else if (this.warpedImage == null) {
      final WarpFilter warpFilter = getWarpFilter();
      this.warpedImage = warpFilter.filter(this.image);
    }
  ...
  public synchronized WarpFilter getWarpFilter() {
    if (this.image != null && this.warpFilter == null) {
      ...
      if (this.boundingBox.isEmpty()) {
        this.warpFilter = new WarpAffineFilter(this.boundingBox, imageWidth,
          imageHeight);
      } else {
        List<MappedLocation> mappings = getTiePoints();
        if (mappings.isEmpty()) {
        ...
        this.warpFilter = WarpFilter.createWarpFilter(this.boundingBox,
          mappings, this.degree, imageWidth, imageHeight);
      }
    }
  ...
  public void setJaiImage(final PlanarImage jaiImage) {

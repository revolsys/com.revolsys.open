...
public class EnhancedRandomAccessFile implements DataInput, DataOutput {
   ...
   * @throws IOException
   */
  public EnhancedRandomAccessFile(final File file, final String mode)
    throws IOException {
   ...
   *            _more_
   */
  protected EnhancedRandomAccessFile(final int bufferSize) {
    eraf = null;
    readonly = true;
    init(bufferSize);
  }
  ...
  public long length() throws IOException {
    final long fileLength = eraf.length();
   ...
   *             _more_
   */
  protected int read_(final long pos, final byte[] b, final int offset,
   ...
   *                if an I/O error occurrs.
   */
  protected int readBytes(final byte b[], final int off, final int len)
    throws IOException {
   ...
   *             _more_
   */
  public void readDouble(final double[] pa, final int start, final int n)
    throws IOException {
   ...
   *             _more_
   */
  public void readFloat(final float[] pa, final int start, final int n)
    throws IOException {
    for (int i = start, end = n + start; i < end; i++) {
   ...
   *                if an I/O error occurs.
   */
  public void readInt(final int[] pa, final int start, final int n)
    throws IOException {
    for (int i = start, end = n + start; i < end; i++) {
      pa[i] = readInt();
    }
   ...
   * @throws IOException
   */
  public int readIntUnbuffered(final long pos) throws IOException {
   ...
   *             _more_
   */
  public void readLong(final long[] pa, final int start, final int n)
    throws IOException {
    for (int i = start, end = n + start; i < end; i++) {
      pa[i] = readLong();
    }
   ...
   *             _more_
   */
  public void readShort(final short[] pa, final int start, final int n)
    throws IOException {
    for (int i = start, end = n + start; i < end; i++) {
      pa[i] = readShort();
    }
  ...
  public String toString() {
    return "fp=" + filePosition + ", bs=" + bufferStart + ", de=" + dataEnd
      + ", ds=" + dataSize + ", bl=" + buffer.length + ", readonly=" + readonly
      + ", bm=" + bufferModified;
  }
   ...
   *             _more_
   */
  public void writeBoolean(final boolean[] pa, final int start, final int n)
    throws IOException {
  ...
  public void writeBytes(final byte b[], final int off, final int len)
    ...
    if (len < buffer.length) {
      ...
      if (spaceInBuffer > 0) {
        ...
        System.arraycopy(b, off, buffer, (int)(filePosition - bufferStart),
          copyLength);
        bufferModified = true;
   ...
   *                if an I/O error occurs.
   */
  public void writeBytes(final char b[], final int off, final int len)
    throws IOException {
   ...
   *             _more_
   */
  public void writeChar(final char[] pa, final int start, final int n)
    throws IOException {
    for (int i = start, end = start + n; i < end; i++) {
      writeChar(pa[i]);
    }
   ...
   *             _more_
   */
  public void writeDouble(final double[] pa, final int start, final int n)
    throws IOException {
    for (int i = start, end = start + n; i < end; i++) {
      writeDouble(pa[i]);
    }
   ...
   *             _more_
   */
  public void writeFloat(final float[] pa, final int start, final int n)
    throws IOException {
    for (int i = start, end = start + n; i < end; i++) {
      writeFloat(pa[i]);
    }
   ...
   *             _more_
   */
  public void writeInt(final int[] pa, final int start, final int n)
    throws IOException {
    for (int i = start, end = start + n; i < end; i++) {
      writeInt(pa[i]);
    }
   ...
   *             _more_
   */
  public void writeLong(final long[] pa, final int start, final int n)
    throws IOException {
    for (int i = start, end = start + n; i < end; i++) {
      writeLong(pa[i]);
    }
   ...
   *             _more_
   */
  public void writeShort(final short[] pa, final int start, final int n)
    throws IOException {
    for (int i = start, end = start + n; i < end; i++) {
      writeShort(pa[i]);
    }

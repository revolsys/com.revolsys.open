...
public class ImageIOUtilities {
  ...
  public final static void checkNotNull(final Object checkMe,
    final String message) {
    if (checkMe == null) {
      throw new IllegalArgumentException(message != null ? message
        : "The provided object was NULL");
    }
  }
  ...
  public static final ColorModel createColorModel(final SampleModel sampleModel) {
    ...
    if (sampleModel instanceof ComponentSampleModel) {
      ...
      final boolean hasAlpha = (numBands == 2) || (numBands == 4);
      final boolean isAlphaPremultiplied = false;
      final int transparency = hasAlpha ? Transparency.TRANSLUCENT
        : Transparency.OPAQUE;
      if (dataType == DataBuffer.TYPE_SHORT) {
        colorModel = new ComponentColorModel(colorSpace, sampleSize, hasAlpha,
          isAlphaPremultiplied, transparency, dataType);
      } else {
        colorModel = RasterFactory.createComponentColorModel(dataType,
      ...
      final int[] bitMasks = sppsm.getBitMasks();
      int rmask = 0;
      int gmask = 0;
      int bmask = 0;
      int amask = 0;

      final int numBands = bitMasks.length;
      if (numBands <= 2) {
      ...
      final int bitsPerSample = sampleSize[0];
      final int numEntries = 1 << bitsPerSample;
      final byte[] map = new byte[numEntries];
   ...
   */
  public static void displayImageIOMetadata(final Node root) {
    displayMetadata(root, 0);
  }

  static void displayMetadata(final Node node, final int level) {
  ...
  public static void disposeImage(RenderedImage rOp) {
    if (rOp != null) {
      if (rOp instanceof RenderedOp) {
        final RenderedOp renderedOp = (RenderedOp)rOp;
        ...
        if (nSources > 0) {
          ...
          // get the reader
          final Object imageReader = rOp.getProperty(ImageReadDescriptor.PROPERTY_NAME_IMAGE_READER);
  ...
  public static List<ImageReaderWriterSpi> getJDKImageReaderWriterSPI(
    final ServiceRegistry registry, final String formatName,
    ...
    final Iterator<? extends ImageReaderWriterSpi> iter = iioRegistry.getServiceProviders(
      spiClass, true); // useOrdering

    String formatNames[];
    ImageReaderWriterSpi provider;
    final String desc = "standard " + formatName + descPart;
    final String jiioPath = "com.sun.media.imageioimpl";
   ...
   * @return 
   */
  public static int getSubSamplingFactor2(final int xSubsamplingFactor,
    ...
    resamplingIsRequired = subSamplingFactorsAreDifferent
      ...
      || optimalSubsampling != newSubSamplingFactor;
  ...
  static void indent(final int level) {
  ...
  public static boolean replaceProvider(
    ...
    final String customProviderName, final String originalProviderName,
    final String format) {
    // now we need to set the order
    final IIORegistry registry = IIORegistry.getDefaultInstance();
    ImageReaderWriterSpi standard = null, custom = null;

    for (final Iterator<? extends ImageReaderWriterSpi> it = registry.getServiceProviders(
      providerClass, false); it.hasNext();) {
        ...
        if (formats[i].equalsIgnoreCase(format)) {
          ...
          if (standard != null && custom != null) {
            if (ImageReaderSpi.class.isAssignableFrom(standard.getClass())) {
              return registry.setOrdering(ImageReaderSpi.class,
                (ImageReaderSpi)custom, (ImageReaderSpi)standard);
            } else {
              return registry.setOrdering(ImageWriterSpi.class,
                (ImageWriterSpi)custom, (ImageWriterSpi)standard);
            }
          }
        }
      }
  ...
  public static boolean sameInterfaces(final Class<?> object1,
    ...
    /*
     * For each interface assignable to 'base' in the 'c1' array, check if this
     * interface exists also in the 'c2' array. Order doesn't matter.
     */
   ...
   */
  public static void setNativeAccelerationAllowed(final String operation,
   ...
   */
  public synchronized static void setNativeAccelerationAllowed(
    ...
    if (factories != null) {
      ...
      if (currentState != null && currentState.booleanValue() != allowed) {
        RIFRegistry.unsetPreference(registry, operation, product,
          allowed ? javaFactory : nativeFactory, allowed ? nativeFactory
            : javaFactory);
        RIFRegistry.setPreference(registry, operation, product,
          allowed ? nativeFactory : javaFactory, allowed ? javaFactory
            : nativeFactory);
      }
    }
  }
   ...
   */
  public static void visualize(final RenderedImage ri, final String title) {
   ...
   */
  public static void visualize(final RenderedImage ri, final String title,
    final boolean rescale, final int roiThreshold) {
    if (rescale == true) {
   ...
   */
  static void visualizeRescaled(final RenderedImage image, final String title,
    final int roiThreshold) {

    final ROI roi = new ROI(image, roiThreshold);
    ...
    final double[] scale = new double[] {
      (255) / (extrema[1][0] - extrema[0][0])
    };
    final double[] offset = new double[] {
      ((255) * extrema[0][0]) / (extrema[0][0] - extrema[1][0])
    };
    ...
    final RenderedOp rescaledImage = JAI.create("Rescale", pbRescale);

    final ParameterBlock pbConvert = new ParameterBlock();
    pbConvert.addSource(rescaledImage);
    pbConvert.add(DataBuffer.TYPE_BYTE);
    final RenderedOp destImage = JAI.create("format", pbConvert);

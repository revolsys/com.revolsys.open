...
public class GDALCommonIIOImageMetadata extends CoreCommonImageMetadata {
  ...
  private static final Logger LOG = LoggerFactory.getLogger(GDALCommonIIOImageMetadata.class.toString());
  ...
  public GDALCommonIIOImageMetadata(final Dataset dataset, final String name,
    ...
    try {
      ...
      //
      // //
      final Map<String, String> defMap = dataset.GetMetadata_Dict(GDALUtilities.GDALMetadataDomain.DEFAULT);
      if (defMap != null && defMap.size() > 0) {
        gdalDomainMetadataMap.put(
          GDALUtilities.GDALMetadataDomain.DEFAULT_KEY_MAP, defMap);
      }

      final Map<String, String> imageStMap = dataset.GetMetadata_Dict(GDALUtilities.GDALMetadataDomain.IMAGESTRUCTURE);
      if (imageStMap != null && imageStMap.size() > 0) {
        gdalDomainMetadataMap.put(
          GDALUtilities.GDALMetadataDomain.IMAGESTRUCTURE, imageStMap);
      }
   ...
   *                open.
   */
  public GDALCommonIIOImageMetadata(final String sDatasetName) {
  ...
  public GDALWritableCommonIIOImageMetadata asWritable() {
    final GDALWritableCommonIIOImageMetadata metadata = new GDALWritableCommonIIOImageMetadata(
      this.getDatasetName());
    ...
    if (this.gdalDomainMetadataMap != null) {
      final Map<String, Map<String, String>> inputMap = this.gdalDomainMetadataMap;
      ...
      while (outKeys.hasNext()) {
        ...
        while (inKeys.hasNext()) {
          final String ikey = inKeys.next();
          final String value = valuesMap.get(ikey);
    ...
    final SampleModel sm = this.getSampleModel();
    if (sm != null) {
    ...
    if (cm != null) {
      if (cm instanceof IndexColorModel) {
        ...
        if (icm.hasAlpha()) {
          final byte[] a = new byte[mapSize];
          icm.getAlphas(a);
          metadata.setColorModel(new IndexColorModel(icm.getPixelSize(),
            mapSize, r, g, b, a));
        } else {
          metadata.setColorModel(new IndexColorModel(icm.getPixelSize(),
            mapSize, r, g, b));
  ...
  public List<GCP> getGCPs() {
    if (super.getGCPs().isEmpty()) {
      ...
      try {
        ...
        final List<org.gdal.gdal.GCP> gcps = new Vector<org.gdal.gdal.GCP>(nGCP);
        ds = GDALUtilities.acquireDataSet(getDatasetName(),
  ...
  protected Map getGdalMetadataDomain(final String metadataDomain) {
    ...
    } else if (metadataDomain.equalsIgnoreCase(GDALUtilities.GDALMetadataDomain.IMAGESTRUCTURE)
      || metadataDomain.startsWith(GDALUtilities.GDALMetadataDomain.XML_PREFIX)) {
   ...
   * @param dataset
   */
  private void performTileSizeTuning(final Dataset dataset) {
   ...
   *         initialized
   */
  private boolean setMembers(final Dataset dataset) {
    ...
    try {
      ...
      if (tileSize < 0) {
        ...
        setSampleModel(new PixelInterleavedSampleModel(buffer_type, tileWidth,
          tileHeight, numBands, tileWidth * numBands, bandsOffset));
      }
      ...
      if (colorInterpretations[0] == gdalconstConstants.GCI_PaletteIndex) {
        ...
        try {
          ...
          final IndexColorModel icm = ct.getIndexColorModel(gdal.GetDataTypeSize(buf_type));
          setColorModel(icm);
        } finally {
          if (ct != null) {
            try {

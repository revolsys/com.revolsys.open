...
public class OgrQueryIterator extends AbstractIterator<Record> {
  ...
  protected Calendar getCalendar(final Feature feature, final int fieldIndex) {
    ...
    feature.GetFieldAsDateTime(fieldIndex, year, month, day, hour, minute,
      second, timeZoneId);
    TimeZone timeZone;
    if (timeZoneId[0] == 100) {
  ...
  protected double[] getCoordinates(final org.gdal.ogr.Geometry ogrGeometry,
    final int axisCount) {
      ...
      if (feature == null) {
        ...
        try {
          ...
          for (int geometryIndex = 0; geometryIndex < geometryCount; geometryIndex++) {
            final String fieldName = feature.GetGeomFieldDefnRef(geometryIndex)
                .GetName();
